...
// Create self-signed x509 certificate using private key
...
pub fn prepare_quic_config(config: &Config) -> Result<quiche::Config> {
    ...
    let mut context = SslContextBuilder::new(SslMethod::tls())
        .map_err(|e| anyhow::anyhow!("new ssl context: {e}"))?;

    context
        .set_private_key(&priv_key)
        .map_err(|e| anyhow::anyhow!("set private key: {e}"))?;

    context
        .set_certificate(&cert)
        .map_err(|e| anyhow::anyhow!("set certificate: {e}"))?;
    ...
    context
        ...
        .unwrap_or_else(|e| log::error!("set sigalgs list: {e}"));
